...
import { Anniversary, EventApiRequest } from "../src/interface";
...
describe("createText()", (): void => {
  ...
  it("記念日の経過日数を返す", async () => {
    expect(createText("ダミー記念日", "2018/01/01")).toBe(
      "ダミー記念日(2018/01/01): 1年1ヶ月14日"
    );
  });
});
...
describe("isPostMessage()", (): void => {
  it("slack通知するevent", async () => {
    const eventParams: EventApiRequest = {
      event: {
        text: "記念日",
        type: "message",
